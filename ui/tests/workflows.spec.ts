...
test.describe('Chat Flow Tests', () => {
  ...
  test('complete chat workflow', async ({ page }) => {
    ...
    await expect(page.locator('.bg-blue-600').locator('text=Hello, how are you?')).toBeVisible();
  ...
  test('timeout handling workflow', async ({ page }) => {
    ...
    await expect(page.locator('.text-red-700')).toContainText('request timeout', { timeout: 15000 });
  });
  ...
  test('multiple messages workflow', async ({ page }) => {
    ...
    for (const message of messages) {
      ...
      await expect(page.locator(`.bg-blue-600:has-text("${message}")`)).toBeVisible();
    }
