...
class ChatStreamChunk(BaseModel):
    ...
    type: str = Field(..., description="Type of chunk: 'search', 'reformulate', 'start', 'content', 'citations', 'usage', 'end'")
    content: Optional[str] = Field(default=None, description="Content for content chunks")
    citations: Optional[List[ChatCitation]] = Field(default=None, description="Citations data")
    usage: Optional[ChatUsage] = Field(default=None, description="Usage information")
    timing_seconds: Optional[float] = Field(default=None, description="Timing information in seconds")
    search_results_count: Optional[int] = Field(default=None, description="Number of search results")
    reformulated_query: Optional[str] = Field(default=None, description="Reformulated query based on history")
...
chat_rate_limiter = ChatRateLimiter(
    max_requests=settings.chat_rate_limit_rpm,
    window_seconds=60
)
...
class ContextAssembler:
    ...
    def _format_chunk_header(self, result: SearchResult, citation_num: int) -> str:
        ...
        return f"[{citation_num}] {chat_title} — {date_str} — message {result.message_id}:"
...
class QueryReformulator:
    ...
    async def reformulate_query(self, question: str, history: List[ChatMessage], model: str) -> str:
...
class ChatService:
    ...
    def _load_system_prompt(self) -> str:
        ...
        try:
            ...
            raise RuntimeError(f"Failed reading system prompt file {prompt_path}: {e}") from e
        if not content:
    ...
    async def chat_stream(self, request: ChatRequest, user_id: str) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            if not search_results:
                ...
                end_chunk = ChatStreamChunk(
                    type="end",
                    citations=[],
                    usage=ChatUsage(prompt_tokens=0, completion_tokens=0, total_tokens=0, cost_usd=0.0),
            ...
            system_message = self.system_prompt.format(current_datetime=current_datetime)
            ...
            if request.history:
                ...
                history_to_include = request.history[-16:]  # Last 8 exchanges (16 messages)
            ...
            full_response_content = ""  # Accumulate streamed content for manual token counting fallback
            ...
            if usage_data is None:
                try:
                    ...
                    prompt_tokens_raw = 0
                    message_overhead = 4
                    system_message = self.system_prompt.format(current_datetime=datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
                    prompt_messages: List[dict[str, str]] = [{"role": "system", "content": system_message}]
                    if request.history:
                    ...
                    context_for_count, _ = assembler_for_count.assemble_context([])  # empty context for simplicity
                    ...
                    for m in prompt_messages:
                        prompt_tokens_raw += len(tokenizer.encode(m["content"])) + message_overhead
                    prompt_tokens = prompt_tokens_raw + 2  # final priming
                    completion_tokens = len(tokenizer.encode(full_response_content)) if content_received else 0
                    total_tokens = prompt_tokens + completion_tokens
                    cost = self.cost_estimator.estimate_cost(model_id, prompt_tokens, completion_tokens)
                    usage_data = ChatUsage(
            ...
            # Send citations
            citations_chunk = ChatStreamChunk(
                type="citations",
                citations=citations
            )
            ...
            end_chunk = ChatStreamChunk(
                type="end",
                usage=usage_data or ChatUsage(prompt_tokens=0, completion_tokens=0, total_tokens=0, cost_usd=0.0),
    ...
    def _build_search_request(self, chat_request: ChatRequest, query: str = None) -> SearchRequest:
        ...
        if chat_request.filters:
            ...
            if chat_request.filters.chat_ids and len(chat_request.filters.chat_ids) > 0:
                search_req.chat_id = chat_request.filters.chat_ids[0]  # Use first chat_id
