...
from telethon.tl.types import (
    ...
    MessageActionChatDeleteUser, MessageActionChatAddUser
)
...
class TelethonClientWrapper:
    ...
    async def start(self):
        ...
        logger.info(f"Connected as {self.me.first_name} (@{self.me.username or 'no_username'})")
        ...
        for name in chat_names:
            try:
                if name == "<Saved Messages>":
                    ...
                    title = getattr(entity, 'title', None) or getattr(entity, 'first_name', name)
                ...
                logger.info(f"Resolved '{name}' -> {resolved[name]['title']} ({resolved[name]['type']})")
    ...
    async def get_messages(
        ...
        try:
            ...
            async for message in self.client.iter_messages(entity, **kwargs):
                ...
                if not message.text and not (message.media and hasattr(message.media, 'caption')):
                    continue
    ...
    async def get_message_by_id(self, entity: Any, message_id: int) -> Optional[Message]:
    ...
    def extract_message_data(self, message: Message, chat_entity: Any) -> Dict[str, Any]:
