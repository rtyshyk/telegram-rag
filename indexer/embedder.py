...
class Embedder:
    ...
    async def embed_texts(
        ...
        logger.info(f"Need to embed {len(texts_to_embed)} texts, {len(results)} from cache")
        
        # Estimate cost and tokens
        total_tokens = sum(len(text.split()) * 1.3 for text, _ in texts_to_embed)  # rough estimate
        estimated_cost = (total_tokens / 1000) * self.price_per_1k_tokens.get(self.model, 0.0001)
        
        self.metrics.total_tokens += int(total_tokens)
        self.metrics.cost_estimate += estimated_cost
        
        logger.info(f"Estimated cost: ${estimated_cost:.4f} for {total_tokens:.0f} tokens")
        
        if dry_run:
            logger.info("Dry run mode - skipping actual embedding")
            return results
        # Deterministic budget check (simplified to avoid flaky threshold logic)
        budget = self._coerce_float(getattr(settings, 'daily_embed_budget_usd', 0.0), 0.0)
        ...
        async with semaphore:
            ...
            for attempt in range(3):
                try:
                    ...
                    base_ms = self._coerce_float(getattr(settings, 'backoff_base_ms', 500), 500.0)
